...
sound and two subtly broken — sort them and say what gave each one away" is an activity.

Don't take the first type that fits. Two candidates for the same goal should usually come from
...
gloss per unfamiliar term, three questions written down, a one-paragraph summary from memory
afterwards. _Watch out:_ without an obligation this is the most passive thing on the list and
the easiest to mistake for progress.

**Skim for shape.** Headings, first sentences, figures — then sketch what the territory
contains before reading any of it. _Good for:_ deciding what to read properly, and giving the
learner somewhere to put things. _Watch out:_ useless without a follow-up; it's a map, not a
visit.
...
more work than examples — someone who can't say what _isn't_ an instance hasn't got the concept
yet. _Needs:_ near-misses that are genuinely near. An obvious wrong answer teaches nothing.

**Sort a mixed set.** Six or eight items, classify them, then say what rule you used. _Good
for:_ forcing an implicit criterion into words. The stated rule is usually more revealing than
the sorting.

**Compare two cases.** What's the same, what differs, and what does the difference cause? _Good
for:_ the point where a learner knows one instance well and needs the general shape.

**Translate between representations.** Words to diagram, diagram to words, table to narrative,
example to rule, rule to example. _Good for:_ almost everything, and cheap to generate. Each
direction is a different activity and they fail differently — reading someone else's diagram is
not the same skill as drawing one.

**Predict, then check.** Say what will happen before being shown. _Good for:_ making a wrong
model visible to its owner, which nothing else on this list does as well. _Needs:_ the
...
and usually the cheapest first step toward production. _Watch out:_ if the instance is plainly
good or plainly broken, there's no judgment being exercised.

**Critique a plausible flawed artifact.** Like judging, but the flaw is subtle and deliberate,
and the learner doesn't know how many there are — or whether there are any. _Tutor role:_
critique target. _Watch out:_ a learner who finds nothing has either understood or given up,
and you can't tell which without asking.

**Repair.** Fix a broken instance. _Good for:_ the step between judging and producing — harder
...
_Needs:_ the audience to ask questions, or it's just a recitation.
